...
void readCity( FILE *fp, struct City *cty ) {
	...
	//cty->lat = 2;
	//cty->lon = 3;
  fscanf( fp, "%s%lf%lf", (cty -> cityName), &(cty -> lat), &(cty -> lon) );
...
int main( int argc, char *argv[] )
{
  ...
  struct City **cityArray = (struct City **)malloc( n * sizeof( struct City *) );
  // Use the readCity function to fill in the fields of all the cities.
  int count = 0;
  while (count < n) {
  	(*(cityArray + count)) = malloc(sizeof(struct City) );
  	count++;
  }
  count = 0;
  while ( count < n ) {
  		readCity(fp, (*(cityArray + count) ) );
  		count++;
  }
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    char *city;
    while ( count < n) {
    	double newDistance = globalDistance(lat, lon, (*( cityArray + ( count ) )) -> lat, (*( cityArray + (count) )) -> lon );
    	if ( distance > newDistance ) {
    		distance = newDistance;
    		city = ((*(cityArray + (count))) -> cityName);
    	}
    	count++;
    }
    printf("%s %.2lf\n", city, distance);
