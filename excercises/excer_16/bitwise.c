...
{
	unsigned short mask = 0x80;  //0000000010000000
	unsigned short inV = *v;
	if ( ((inV & mask) == 0) && flag) { //bit 7 is 0 want to set it
		mask = 0x80;
		*v=(inV|mask);
		return;
	}
  if ( ( ( inV & mask ) == 0) && flag ) { //bit 7 is 0 and don't want to set it
  	return;
  }
  if ( ((inV & mask) != 0) && flag) { //bit 7 is 1 and you want to set it
  	return;
  }
  if(((inV & mask) != 0) && !flag) { //bit 7 is one and you want to clear it
  	mask = 0xFF7F;
  	*v=(inV&mask);
  	return;
  }
